...
async function runAgent({ input, customerName, customerEmail }) {
  ...
  if (process.env.MODEL_MODE === "local") {
    console.log(`[System] Switching to LOCAL OLLAMA (${process.env.OLLAMA_MODEL_NAME || "gemma2"})...`);
    model = new ChatOllama({
      model: process.env.OLLAMA_MODEL_NAME || "gemma4",
      temperature: 0,
...
2. PROBLEM LOGGING (GUARDRAIL): If the user describes a problem (e.g., "my account is hacked", "payment failed", "login issue") WITHOUT explicitly asking for a ticket first:
   ...
   - At the very end of your response, strictly ask this confirmation line: "Kya aap is dikkat ke liye ek support ticket raise karna chahte hain?"
...
- Provide a clear and friendly final answer.`,
  ...
  try {
    ...
    while (keepRunning && iterations < maxIterations) {
      iterations++;
      console.log(`[Agent - Mode: ${process.env.MODEL_MODE || 'cloud'}] Running iteration ${iterations}...`);
      ...
      if (response.tool_calls && response.tool_calls.length > 0) {
        ...
        if (activeTool) {
          try {
            ...
            console.error(`Tool ${toolCall.name} execution failed:`, toolErr.message);
    ...
    if (iterations >= maxIterations && keepRunning) {
      console.warn(`[Agent] Max iterations (${maxIterations}) reached without a clean exit.`);
      reply = "I'm having trouble retrieving all the details right now. Could you please try again or simplify your request?";
    }
  } catch (err) {
